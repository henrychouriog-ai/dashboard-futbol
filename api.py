...
def obtener_temporada_actual(league_id):
    # Por defecto intentamos 2025, pero el buscador de equipos decidirá la mejor
    return 2025

@st.cache_data(ttl=600)
def obtener_equipos_liga(league_id):
    # INTENTO 1: Temporada 2025 (La más actual en 2026)
    data = _api_get("/teams", params={"league": league_id, "season": 2025})
    res = data.get("response", [])
    
    # INTENTO 2: Temporada 2024 (Si la 2025 está vacía aún)
    if not res:
...
def obtener_promedios_goles(team_id, league_id):
    ...
    # Buscamos en ambas temporadas para asegurar promedios reales
    for yr in [2025, 2024]:
